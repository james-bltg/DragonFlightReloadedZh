...
DFRL:NewMod("Gui-base", 2, function()
    ...
    function Setup:TitleText()
        if not self.dragonText then
            ...
            self.dragonText:SetText(DFRL:TrText("Dragonflight:"))
            ...
            self.reloadedText:SetText(DFRL:TrText("Reloaded"))
    ...
    function Setup:SubFrame()
        if not self.subFrame then
            ...
            self.profileText:SetText(DFRL:TrText("Profile:") .. "   |cffffffff" .. profileName .. "|r")
            ...
            self.subFrame:SetScript("OnUpdate", function()
                ...
                self.fpsText:SetText(DFRL:TrText("FPS:") .. " |cffffffff" .. format("%.1f", GetFramerate()) .. "|r")
                self.profileText:SetText(DFRL:TrText("Profile:") .. "   |cffffffff" .. (DFRL_CUR_PROFILE[UnitName("player")] or "Default") .. "|r")
            end)
    ...
    function Setup:TabButtons()
        if not self.tabsCreated then
            for i = 1, table.getn(Setup.tabs) do
                ...
                tab:SetText(DFRL:TrText(Setup.tabs[i]))
    ...
    function Setup:SelectTab(tabIndex)
        ...
        if tabIndex ~= 1 then
            self.panelTitle:SetText(DFRL:TrText(self.tabs[tabIndex]))
        else
