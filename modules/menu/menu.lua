...
DFRL:NewMod("Menu", 1, function()
    ...
    function Setup:MenuFrame()
        if not self.menuframe then
            ...
            local function getLangLabel()
                local setting = DFRL_DB_SETUP and DFRL_DB_SETUP.language or "auto"
                if setting == "zhCN" then
                    return "Language: Chinese"
                end
                if setting == "enUS" then
                    return "Language: English"
                end
                if DFRL:GetAddonLocale() == "zhCN" then
                    return "Language: Chinese"
                end
                return "Language: English"
            end

            local langBtn = DFRL.tools.CreateButton(self.menuframe, getLangLabel(), self.btnw, 20, true)
            langBtn:SetPoint("TOP", resumeBtn, "BOTTOM", 0, -self.space)
            langBtn:SetScript("OnClick", function()
                DFRL:ToggleAddonLanguage()
            end)
        end
    end
